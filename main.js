...
function init() {
    ...
    // hiBall.add(new THREE.SpotLight(0xff0000));
    // hiBall.add(new THREE.SpotLight(0x0000ff));
    vizGroup.add(hiBall);
    // group everything
    scene.add(vizGroup)
    ...
    // audioLoader.load('11. Motorhead.mp3',
    // audioLoader.load('02 Scary Monsters And Nice Sprites.mp3',
    // audioLoader.load('./1-10 Thiz Rox.mp3',
    audioLoader.load('./voxel_revolution.mp3',
    // audioLoader.load('./audiocheck.net_sweep20-20klin.ogg',
    // audioLoader.load('./phat-sketch-by-kevin-macleod-from-filmmusic-io.mp3',
    buffer=>{
...
function render(time) {
    ...
    // vizGroup.rotation.y=Math.sin(time)*.5;
    // vizGroup.position.x=Math.sin(time)*8;
    // vizGroup.position.z=Math.cos(time)*8;
